    ...
    it('should create and return a new item when provided valid title and content', function () {
      ...
      return chai.request(app)
        ...
        .then(function (_res) {
          ...
          expect(_res).to.have.header('location');
          expect(_res).to.be.json;
          expect(body).to.be.a('object');
          expect(body).to.have.all.keys('id', 'title', 'content', 'createdAt', 'updatedAt', 'tags', 'userId');
          return Note.findById({_id: body.id, userId: user.id});
        })
        .then(data => {
    ...
    it('should update the note when provided a valid title', function () {
      ...
      return Note.findOne()
        ...
        .then(function (res) {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          // expect(res.body).to.include.keys('id', 'title', 'content', 'createdAt', 'folderId', 'updatedAt', 'userId', 'tags');
          expect(res.body.id).to.equal(data.id);
